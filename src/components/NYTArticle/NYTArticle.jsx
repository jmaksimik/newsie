...
export default function NYTArticle({ article, removeBookmark, bookmarks, addBookmark }) {
    ...
    // NYT specific handlers for bookmark model
    async function handleNytBookmark() {
        const bookmark = {
    ...
    const nytClickHandler = bookmarkIndex > -1 ?
        () => removeBookmark(bookmarks[bookmarkIndex]._id) : () => handleNytBookmark(article._id)

    // Guardian specific handlers for bookmark model
    async function handleGuardianBookmark() {
        const bookmark = {
            title: article.webTitle,
            url: article.webUrl,
    ...
    const guardianClickHandler = bookmarkIndex > -1 ?
        () => removeBookmark(bookmarks[bookmarkIndex]._id) : () => handleGuardianBookmark(article._id)
